...
class SearchDetailView(APIView):
    ...
    def get(self, request, language, search):
        ...
        if language == 'ru':
            ...
            education = Conference.objects.filter(ru_search, draft=False)
            news = News.objects.filter(Q(title__icontains=search) | Q(text__icontains=search), draft=False)
            society = Society.objects.filter(ru_search, draft=False)
            protocols = Protocols.objects.filter(ru_search, draft=False)
            event = Event.objects.filter(ru_search, draft=False)
        elif language == 'kz':
            serializers = SearchListSerializerKZ
            education = Conference.objects.filter(kz_search, draft=False)
            news = News.objects.filter(Q(title__icontains=search) | Q(text__icontains=search), draft=False)
            society = Society.objects.filter(kz_search, draft=False)
            protocols = Protocols.objects.filter(kz_search, draft=False)
            event = Event.objects.filter(kz_search, draft=False)
        else:
            serializers = SearchListSerializerEN
            education = Conference.objects.filter(en_search, draft=False)
            news = News.objects.filter(Q(title__icontains=search) | Q(text__icontains=search), draft=False)
            society = Society.objects.filter(en_search, draft=False)
            protocols = Protocols.objects.filter(en_search, draft=False)
            event = Event.objects.filter(en_search, draft=False)
